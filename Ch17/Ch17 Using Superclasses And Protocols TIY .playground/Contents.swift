...
let incubus = AudioCD(title: "Trust Fall (Side A) EP", barcode: "88889999", audioTracks: 4)

let items: [Rentable] = [theNotebook, majorLeague, incubus]

for item in items {
    print("Item: \(item.title) is rentable. Barcode is \(item.barcode).")
    if item.title == "Trust Fall (Side A) EP" {
        print("My favorite song on \(item.title) is Absolution Calling.")
    }
}
